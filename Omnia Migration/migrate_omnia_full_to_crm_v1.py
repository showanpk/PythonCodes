...
def resolve_member(
    ...
    if len(lite_candidates) == 1:
        c = lite_candidates[0]
        if p.dob and c.dob and p.dob != c.dob:
            if not source_knows_distinct_dobs(c.dob):
                return None, f"Exact Lite name match '{p.name}' has conflicting DOB ({p.dob} vs {c.dob})."
            # Source itself proves two distinct people share this name; fall through
            # and create a second Lite identity using DOB as the discriminator.
            lite_candidates = []
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Saheli CRM Omnia Medical Practice historical migration")
