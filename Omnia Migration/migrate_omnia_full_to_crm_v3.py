...
# Abida Bi appears in 2023/2024 with DOB 15/12/1976 and later in the same Omnia
# source with Saheli Card 210 + DOB 15/12/1967. The year digits are transposed.
# Earlier rows may inherit card 210 ONLY if the live CRM confirms card 210 belongs
# to Abida Bi and its DOB is the later source DOB 1967-12-15.
TRANSPOSED_DOB_CARD_REVIEW = {
    ("abida bi", "210"): {
        "old_dob": date(1976, 12, 15),
        "canonical_dob": date(1967, 12, 15),
        "expected_crm_name": "abida bi",
    },
}
    ...
    for p in people:
        ...
        if p.dob and card_dobs and p.dob not in card_dobs:
            if len(card_dobs) == 1 and years_are_transposed(p.dob, next(iter(card_dobs))):
                ...
                review_rule = TRANSPOSED_DOB_CARD_REVIEW.get((p.name_norm, card))
                if (
                    review_rule
                    and p.dob == review_rule["old_dob"]
                    and target_dob == review_rule["canonical_dob"]
                ):
                    p.card = card
                    actions.append(Action(
                        action="SOURCE_CARD_RESOLVED_TRANSPOSED_DOB_PENDING_CRM", severity="INFO",
                        source_file=p.source_file, sheet=p.sheet, source_ref=f"Row {p.row}",
                        source_name=p.name, source_card=card,
                        detail=(
                            f"Exact source continuity maps '{p.name}' to card {card}. "
                            f"Older DOB {p.dob} and later card-profile DOB {target_dob} have transposed year digits. "
                            "Card is assigned for preview, but live CRM name/DOB must confirm before commit."
                        ),
                    ))
                else:
                    actions.append(Action(
