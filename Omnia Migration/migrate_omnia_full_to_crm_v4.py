...
# No broad fuzzy exceptions are allowed. These are narrow, evidence-backed
# exceptions confirmed from the Omnia source plus the live CRM preview.
SAFE_REVIEWED_CARD_NAME_REUSE = {
    # V2 live preview: source Card 1328 / "Ghazal Yasmin" resolves to the existing
    # CRM card-holder "Ghazala Yasmin Mahroof". No CRM profile fields are overwritten.
    ("1328", "ghazal yasmin"): "ghazala yasmin mahroof",
}
SAFE_EXACT_FULL_NAME_REUSE = set()

# Narrow reviewed source-vs-CRM DOB mismatch for an already card-bearing row.
# Card and exact name are the identity evidence; CRM profile remains authoritative
# and is never overwritten by the historical source DOB.
SAFE_REVIEWED_CARD_DOB_MISMATCH = {
    ("210", "abida bi", date(1967, 12, 15), date(1966, 12, 15)),
}

# Abida Bi has conflicting DOB years across the historic source and live CRM:
# older no-card rows = 15/12/1976; later source card 210 row = 15/12/1967;
# live CRM card 210 = 15/12/1966. There is not enough evidence to merge the
# older no-card history into FULL card 210. V4 therefore preserves the older
# 1976 identity as a separate Lite member while retaining card 210 for the later
# card-bearing source rows. Existing CRM profile data is never overwritten.
TRANSPOSED_DOB_CARD_REVIEW = {}
...
def enrich_missing_cards_profile_aware(people: Iterable[PersonObservation], name_card_people: dict[str, list[PersonObservation]]) -> list[Action]:
    ...
    for p in people:
        ...
        if p.dob and card_dobs and p.dob not in card_dobs:
            # Do not inherit a Saheli Card across materially conflicting DOBs.
            # This is especially important for Abida Bi: source history contains
            # 1976 and 1967 while live CRM card 210 contains 1966. Preserve the
            # older no-card identity as Lite instead of asserting they are one person.
            actions.append(Action(
                action="SOURCE_SAME_NAME_DIFFERENT_DOB_KEPT_SEPARATE", severity="INFO",
                source_file=p.source_file, sheet=p.sheet, source_ref=f"Row {p.row}",
                source_name=p.name, source_card=card,
                detail=f"Exact name also appears with card {card}, but DOB {p.dob} differs from card-profile DOB(s) {sorted(card_dobs)}. Kept as a separate no-card identity; no CRM profile is overwritten.",
            ))
            continue
...
def resolve_member(
    ...
    if p.card:
        ...
        if existing:
            reviewed_action = "REUSE_FULL_BY_CARD"
            reviewed_detail = f"ParticipantID={existing.participant_id}"

            # Narrow live-CRM validation for the Abida Bi transposed-DOB source case.
            transposed_rule = TRANSPOSED_DOB_CARD_REVIEW.get((p.name_norm, p.card))
            if transposed_rule and p.dob == transposed_rule["old_dob"]:
                if norm_name(existing.full_name) != transposed_rule["expected_crm_name"]:
                    return None, (
                        f"Saheli Card {p.card} transposed-DOB review failed: CRM ParticipantID={existing.participant_id} "
                        f"is '{existing.full_name}', expected Abida Bi."
                    )
                if existing.dob != transposed_rule["canonical_dob"]:
                    return None, (
                        f"Saheli Card {p.card} transposed-DOB review failed: CRM DOB={existing.dob or 'NULL'}, "
                        f"expected {transposed_rule['canonical_dob']} from the later card-bearing Omnia source row."
                    )
                reviewed_action = "REUSE_FULL_BY_REVIEWED_TRANSPOSED_DOB"
                reviewed_detail = (
                    f"ParticipantID={existing.participant_id}; source DOB {p.dob} treated as transposed-year "
                    f"variant of CRM/later-source DOB {existing.dob}; CRM profile not overwritten."
                )

            if p.name_norm and existing.full_name:
                sim = name_similarity(p.name, existing.full_name)
                if sim < 0.80:
                    expected_name = SAFE_REVIEWED_CARD_NAME_REUSE.get((p.card, p.name_norm))
                    if expected_name and norm_name(existing.full_name) == expected_name:
                        reviewed_action = "REUSE_FULL_BY_REVIEWED_CARD_NAME"
                        reviewed_detail = (
                            f"ParticipantID={existing.participant_id}; reviewed source name '{p.name}' "
                            f"matches existing card-holder '{existing.full_name}' for Saheli Card {p.card}; "
                            "CRM profile not overwritten."
                        )
                    else:
                        return None, (
            ...
            # Preserve a narrowly reviewed DOB discrepancy on an already card-bearing
            # source row without changing the CRM participant profile.
            dob_key = (p.card, p.name_norm, p.dob, existing.dob)
            if dob_key in SAFE_REVIEWED_CARD_DOB_MISMATCH:
                reviewed_action = "REUSE_FULL_BY_REVIEWED_CARD_DOB_MISMATCH"
                reviewed_detail = (
                    f"ParticipantID={existing.participant_id}; source card/name match CRM, but source DOB {p.dob} "
                    f"differs from CRM DOB {existing.dob}. CRM profile retained unchanged; historical discrepancy logged."
                )
            m = ResolvedMember(
                ...
                action=reviewed_action, detail=reviewed_detail,
            )
            cache[cache_key] = m
            return m, None
        ...
        if transposed_rule:
            return None, (
                f"Saheli Card {p.card} is not present in CRM, so the transposed-DOB source correction for '{p.name}' "
                "cannot be validated safely. Refusing to create a FULL participant from the disputed older DOB."
            )
...
def run_db_preview_or_commit(args, sessions, empty, source_conflicts, source_actions, source_distinct_name_dobs):
    ...
    try:
        ...
        # Track sessions created earlier in THIS preview/commit transaction. They must
        # never be mistaken for pre-existing CRM conflicts when the source has two
        # genuine time slots on the same date (e.g. 10:30/11:00 and 12:30).
        ...
        for s in sorted(sessions, key=lambda x: (x.session_date, x.start_time, x.activity_name)):
            ...
            if s.key in session_cache:
                ...
                # IMPORTANT: a previous source slot may have just been inserted inside
                # this same transaction. That row is not a pre-existing CRM session and
                # must not block a second genuine source slot at another time.
                baseline_candidates = [
                    c for c in candidates
                    if int(c["SessionId"]) not in created_session_ids
                ]
                chosen, problem = choose_existing_session(baseline_candidates, s)
                if problem:
