#!/usr/bin/env python3
"""
Saheli CRM - Men's Programme Full Historical Migration V3
==========================================================
...
# Existing CRM programme history confirms which duplicate-name Lite record is
# already used for Men's Know Your Number. These overrides are deliberately
# limited to that programme at Bahu Trust; they are not global name mappings.
MEN_KYN_LITE_IDENTITY_OVERRIDES = {
    "mohammed|bashir": "LITE-21",
    "qurban|ali": "LITE-24",
}
...
def clean_kyn_person_name(value: Any) -> Optional[str]:
    """Normalize KYN-only staff annotations without changing the participant identity.

    In the Zaheer register, a small block of names is suffixed with "Rc"
    (for example "Qurban Ali Rc") while the same people appear elsewhere
    without that suffix. Treat the trailing Rc as a source annotation, not
    as part of the person's surname/name, so the migration does not create
    duplicate Lite members solely because of that notation.
    """
    name = clean_text(value)
    if not name:
        return None
    name = re.sub(r"\s+rc\s*$", "", name, flags=re.I).strip()
    return name or None
...
def make_kyn_person(row: tuple[Any, ...]) -> PersonSource:
    ...
    return PersonSource(
        raw_card=None,
        full_name=clean_kyn_person_name(getv(row,6)),
...
def parse_kyn_main(path: Path, start_filter: Optional[date], end_filter: Optional[date]) -> list[SourceSession]:
    ...
    try:
        ...
        for row_num,row in enumerate(ws.iter_rows(min_row=4,max_col=max_needed,values_only=True),start=4):
            name=clean_kyn_person_name(getv(row,6))
...
def resolve_member(cur,p:PersonSource,participants_by_card,full_by_name,lites_by_name,lite_num_state,log,source):
    ...
    # Two duplicate-name Lite pairs are already disambiguated by existing CRM
    # Men's Know Your Number attendance history. Apply only to Bahu Trust KYN
    # rows so this cannot silently affect unrelated activities or venues.
    if (
        len(candidates) > 1
        and source.canonical_activity == "Men's Know Your Number"
        and normalize_venue(source.venue_name) == "bahu trust"
    ):
        preferred_membership = MEN_KYN_LITE_IDENTITY_OVERRIDES.get(nk)
        if preferred_membership:
            preferred = [
                c for c in candidates
                if (clean_text(c.membership_id) or "").upper() == preferred_membership.upper()
            ]
            if len(preferred) == 1:
                lite = preferred[0]
                return (
                    ...
                    lite.lite_id,
                    lite.membership_id,
                    f"{lite.first_name} {lite.last_name}".strip(),
                    "MATCHED_LITE_PROGRAM_HISTORY_OVERRIDE",
                )
    ...
    if candidates and why=="AMBIGUOUS_NAME":
        # Keep the match blocked, but include enough masked candidate detail in
        # the preview CSV to review the duplicate CRM Lite records safely.
        candidate_bits=[]
        for c in sorted(candidates,key=lambda x:x.membership_id):
            dob=c.dob.isoformat() if c.dob else "-"
            postcode=clean_text(c.postcode) or "-"
            phone_digits=normalize_phone(c.phone)
            phone_hint=("***"+phone_digits[-4:]) if len(phone_digits)>=4 else "-"
            candidate_bits.append(
                f"{c.membership_id}[dob={dob},postcode={postcode},phone={phone_hint}]"
            )
        log.add(
            "REVIEW_AMBIGUOUS_LITE_NAME",source,p,
            detail=f"{len(candidates)} existing Lite members share this name; candidates="+" | ".join(candidate_bits)
        )
        return None
...
def run_migration(commit:bool,start_filter:Optional[date],end_filter:Optional[date],audit_only:bool=False):
    ...
    print("Saheli CRM - Men's Full Historical Migration V3")
