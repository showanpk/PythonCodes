...
def make_kyn_person(ws, row: int) -> PersonSource:
    return PersonSource(
        raw_card=None,
        full_name=clean_text(ws.cell(row,6).value),
        dob=safe_dob(ws.cell(row,7).value),
        postcode=clean_text(ws.cell(row,8).value),
        phone=clean_text(ws.cell(row,9).value),
        emergency_name=clean_text(ws.cell(row,10).value),
        emergency_phone=clean_text(ws.cell(row,11).value),
        gender=clean_text(ws.cell(row,2).value),
        ethnicity=clean_text(ws.cell(row,12).value),
        health_conditions=clean_text(ws.cell(row,13).value),
    )
...
def parse_kyn_main(path: Path, start_filter: Optional[date], end_filter: Optional[date]) -> list[SourceSession]:
    ...
    # Attendance/event columns start at X (24). A nonblank value means the
    # person attended; some cells contain BP readings instead of 'Present'.
    for col in range(24, ws.max_column+1):
        dt=mens_source_date(ws.cell(2,col).value,2025)
        if not dt:
            continue
        if start_filter and dt < start_filter: continue
        if end_filter and dt > end_filter: continue
        for row in range(4, ws.max_row+1):
            name=clean_text(ws.cell(row,6).value)
            mark=clean_text(ws.cell(row,col).value)
            venue=kyn_venue(ws.cell(row,5).value)
            if not name or not mark or not venue:
                continue
            key=(dt,normalize_venue(venue))
            s=sessions.get(key)
            if not s:
                if normalize_venue(venue)=="bahu trust":
                ...
                sessions[key]=s
            s.people.append((make_kyn_person(ws,row),f"{get_column_letter(col)}{row}"))
    wb.close()
...
def parse_kyn_continuation(path: Path, start_filter: Optional[date], end_filter: Optional[date]) -> tuple[list[SourceSession],int]:
    ...
    for row in range(2,ws.max_row+1):
        ...
        if clean_text(raw_date) and re.fullmatch(r"19[/.-]05[/.-]206",clean_text(raw_date)):
            corrected+=1
        ...
        p=PersonSource(
            raw_card=None,full_name=name,dob=safe_dob(ws.cell(row,7).value),postcode=clean_text(ws.cell(row,8).value),
            phone=clean_text(ws.cell(row,9).value),emergency_name=clean_text(ws.cell(row,10).value),
            emergency_phone=clean_text(ws.cell(row,11).value),gender=clean_text(ws.cell(row,2).value),
            ethnicity=clean_text(ws.cell(row,12).value),health_conditions=clean_text(ws.cell(row,13).value),
        )
        s.people.append((p,f"ROW-{row}"))
    wb.close(); return list(sessions.values()),corrected
...
def parse_mens_2026(path: Path, start_filter: Optional[date], end_filter: Optional[date]) -> tuple[list[SourceSession],dict[str,int]]:
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in range(2,ws.max_row+1):
            # B-E are sometimes blank on continuation participant rows.
            for col in (2,3,4,5):
                val=ws.cell(row,col).value
                if clean_text(val): carry[col]=val
            dt=mens_source_date(carry[3])
            card=valid_saheli_card(ws.cell(row,6).value)
            name=clean_text(ws.cell(row,7).value)
            if normalize_words(name) in {"n a","na"}: name=None
            raw_card_text=clean_text(ws.cell(row,6).value)
            ...
            p=PersonSource(
                raw_card=card,full_name=name,emergency_name=clean_text(ws.cell(row,8).value),
                emergency_phone=clean_text(ws.cell(row,9).value),risk=parse_risk(ws.cell(row,10).value),
            )
    ...
    wb.close(); return sessions,dict(stats)
