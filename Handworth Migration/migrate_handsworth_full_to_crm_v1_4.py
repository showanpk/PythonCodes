#!/usr/bin/env python3
"""
Saheli CRM - Handsworth historical migration V1.4
...
# Explicitly reviewed source/CRM identity decisions. These are narrow exceptions,
# not generic fuzzy matching rules.
SAFE_REVIEWED_CARD_NAME_REUSE = {
    ("1030", "mandeep kaur"): "CRM 1030 is Mandeep K Dhaliwal at Handsworth; source uses 1030 three times for Mandeep/Madneep Kaur. Reviewed as the same participant despite historical phone/name variation.",
}
...
def apply_source_identity_corrections(people: Iterable[PersonObservation]) -> list[Action]:
    ...
    for p in people:
        ...
        # One late Pilates row contains card 1511 with no usable name. The same
        # participant is repeatedly recorded earlier as 511 / Kashmeer Kaur, and CRM
        # confirms 511 = Kashmir Kaur at Handsworth. Treat 1511 as a one-off source typo.
        if p.card == "1511" and not p.name_norm:
            p.card = "511"
            p.name = "Kashmir Kaur"
            actions.append(Action(
                action="SOURCE_CARD_CORRECTED_TO_511", severity="INFO",
                source_file=p.source_file, sheet=p.sheet, source_ref=f"Row {p.row}",
                source_name=p.name, source_card=p.card,
                detail="Reviewed Handsworth continuity correction: nameless 1511 row resolved to existing card 511 / Kashmir Kaur.",
            ))
...
def resolve_member(
    ...
    if p.card:
        ...
        if existing:
            reviewed_detail = SAFE_REVIEWED_CARD_NAME_REUSE.get((p.card, p.name_norm))
            if p.name_norm and existing.full_name:
                sim = name_similarity(p.name, existing.full_name)
                if sim < 0.80 and not reviewed_detail:
                    return None, (
            ...
            m = ResolvedMember(
                ...
                action="REUSE_FULL_BY_REVIEWED_CARD_NAME" if reviewed_detail else "REUSE_FULL_BY_CARD",
                detail=(f"ParticipantID={existing.participant_id}; {reviewed_detail}" if reviewed_detail else f"ParticipantID={existing.participant_id}"),
            )
            cache[cache_key] = m
            return m, None
