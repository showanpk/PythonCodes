#!/usr/bin/env python3
"""
Saheli CRM - Handsworth historical migration V1.1
...
def parse_sources(source_dir: Path):
    ...
    return delivered, empty, all_people, conflicts, resolution_actions + duplicate_actions + duplicate_session_actions
...
def resolve_member(
    ...
    # No source card: only reuse FULL when exact name + DOB/postcode makes it safe.
            ...
            for a in s.attendance:
                ...
                if existing_att:
                    if bool(existing_att["Attended"]):
                        actions.append(action_from(
                            s, p, "SKIP_EXISTING_ATTENDANCE", f"AttendanceId={existing_att['AttendanceId']} already exists and is attended.",
                            member_kind=member.kind, member_display_id=member.display_id,
                            session_id=session_id, source_ref=a.source_ref
                        ))
                    else:
