...
----------------------------------------------
* May 2025 - October 2025 : Register for Exercise - Calthorpe(1).xlsx (wide monthly format)
* November - December 2025: Calthorpe Register NEW 2025 (2)(1).xlsx (row-per-attendance format)
* January - March 2026    : Calthorpe Register 2026 (4)(1).xlsx (row-per-attendance format)
...
def parse_vertical_workbook(path: Path, min_date: date, max_date: date) -> list[SourceSession]:
    ...
    for ws in wb.worksheets:
        ...
        for excel_row_num, row in enumerate(ws.iter_rows(min_row=2, max_col=10, values_only=True), start=2):
            ...
            grouped[key].people.append((person, f"ROW-{excel_row_num}"))
    wb.close()
...
def create_session(cur, source: SourceSession) -> DbSession:
    ...
    cur.execute(
        ...
        VALUES
            (?, ?, NULL, ?, ?, NULL, NULL, ?, ?,
             0, NULL, ?, NULL, ?, ?, NULL, 0, 0, SYSUTCDATETIME())
        """,
        DEFAULT_SESSION_FREQUENCY[:30], category, category, VENUE_NAME, display, note,
        source.session_date, source.start_time, source.end_time,
    )
    ...
    return DbSession(session_id, source.session_date, VENUE_NAME, display, source.canonical_activity,
                     source.start_time, source.end_time, False, 0)
...
def resolve_member(cur, person: PersonSource, participants_by_card: dict[str, DbParticipant],
    ...
    if candidates:
        ...
        if len(matches) > 1:
            log.add("REVIEW_AMBIGUOUS_COMPOSITE_CARD", source, person, detail=f"Multiple existing cards matched: {[p.card for p in matches]}")
            return None
        if len(candidates) > 1:
    ...
    try:
        ...
        for source in sessions:
            ...
            if db_session:
                ...
                log.add("NEW_SESSION", source, detail=f"Created SessionId={db_session.session_id}; time={db_session.start_time}-{db_session.end_time}; quality={source.time_quality}", session_id=db_session.session_id)
