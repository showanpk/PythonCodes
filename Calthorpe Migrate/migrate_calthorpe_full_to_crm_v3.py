...
# These values appear in the vertical staff registers in the participant/name column
# but describe a non-delivered/cancelled session rather than a person. They must
# NEVER create LiteMembers or attendance rows. The session itself is preserved and
# marked cancelled where safe. Matching is exact after simple normalization so a
# genuine person's name is not accidentally filtered.
...
# Optional explicit fixes for historic cells containing more than one Saheli card.
# Leave empty unless you have verified the correct card in dbo.Participants.
# Example after verification: {"433/727": "727"}
...
def parse_vertical_workbook(path: Path, min_date: date, max_date: date) -> list[SourceSession]:
    ...
    for ws in wb.worksheets:
        ...
        for excel_row_num, row in enumerate(ws.iter_rows(min_row=2, max_col=10, values_only=True), start=2):
            ...
            # Several 2026 source rows place a cardless participant name in the
            # Saheli Card Number column while the formula-based Name cell is #N/A.
...
def insert_attendance(cur, db_session: DbSession, member_kind: str, member_id: str,
    ...
    note = (
        ...
        f"source_session={source.source_ref}; time_quality={source.time_quality}{wellbeing_note}"
    )[:500]
    cur.execute(
        """
...
def resolve_member(cur, person: PersonSource, participants_by_card: dict[str, DbParticipant],
    ...
    candidates = card_candidates(person.raw_card)
    if candidates:
        matches = []
        for c in candidates:
        ...
        if len(matches) == 1:
            p = matches[0]
            return "FULL", str(p.participant_id), p.card, person.full_name or p.full_name, "MATCHED_FULL"
        if len(matches) > 1:
            ...
            # 3) When the source supplies a name, resolve only if exactly one of the
            # candidate cards has the same normalized CRM FullName. No fuzzy guessing.
            ...
            log.add("REVIEW_AMBIGUOUS_COMPOSITE_CARD", source, person, detail=f"Multiple existing cards matched: {[p.card for p in matches]}; source_name={person.full_name or ''}")
            return None
        if len(candidates) > 1:
...
def main() -> int:
    ...
    print("Saheli CRM - Calthorpe Historical Migration")
