#!/usr/bin/env python3
"""
Saheli CRM - Alum Rock (ARCC) full historical migration
========================================================

Reads the uploaded ARCC staff-register formats and migrates them into the current
Saheli CRM schema.  DRY RUN / PREVIEW is the default; use --commit only after
reviewing the generated CSV and ensuring REVIEW_* = 0.

ARCC SPECIAL RULE - INNERVA
----------------------------
Innerva is a booking-slot service.  One unique Session Date + booking time slot is
ONE CRM Session, regardless of how many booked rows are in that slot.  Only rows
explicitly marked Attended=Yes become SessionAttendance.  A valid booking slot with
zero attendees is still a session.  CANCELLED / DO NOT BOOK / BANK HOLIDAY slots
are stored as cancelled sessions and do not create attendance.

CORE ARCC SOURCE FORMATS
------------------------
* 2022-2024 old wide registers: participant rows, session dates/activities across columns.
* 2025 monthly wide register: participant profile rows + Yes marks across dated columns.
* 2025 continuation sheets: one activity per sheet, attendance dates across columns.
* 2026 ARCC activity register: vertical session blocks with participant rows.
  The separate Omnia Chair Exercise sheet is intentionally excluded from ARCC.
* Innerva booking workbook: nine-place booking blocks, grouped by date + time slot.

IDENTITY RULES
--------------
* Saheli Card present -> reuse existing FULL participant by normalized card.
* Valid Saheli Card not found -> create missing FULL participant.
* No Saheli Card -> LITE member.
* LITE match is normalized FirstName + LastName; DOB/postcode/phone can resolve
  duplicate-name matches.  If no safe match exists, create a new LiteMember.
* Existing member/session attendance is never inserted twice.

NOT INCLUDED IN THIS SESSION MIGRATION
--------------------------------------
Appointment Booking Sheet.xlsx is health/assessment appointment scheduling.  It is
reported by --audit-only as excluded and is NOT inserted into Sessions/SessionAttendance.
...
ALL_VENUE_ALIASES = ARCC_VENUE_ALIASES | OMNIA_VENUE_ALIASES
...
MIGRATION_NOTE_PREFIX = "Historical ARCC register import"
...
def is_cancel_text(value: Any) -> bool:
    ...
    return any(marker in w for marker in CANCEL_MARKERS) if w else False
...
def parse_2025_continuation(path: Path, start_filter: Optional[date], end_filter: Optional[date]) -> list[SourceSession]:
    ...
    for ws in wb.worksheets:
        ...
        for ridx,row in enumerate(ws.iter_rows(min_row=3,max_col=max_needed,values_only=True),start=3):
            person=PersonSource(raw_card=getv(row,2), full_name=clean_text(getv(row,3)), emergency_name=clean_text(getv(row,4)), emergency_phone=clean_text(getv(row,5)), risk=parse_risk(getv(row,6)))
...
def validate_connection_string():
    if "YOUR_SQL_" in CONNECTION_STRING or "YOUR_SQL_SERVER" in CONNECTION_STRING:
        raise RuntimeError("Database connection not configured. Edit CONNECTION_STRING or set SAHELI_SQL_CONNECTION_STRING.")
...
def run_migration(commit:bool,start_filter:Optional[date],end_filter:Optional[date],audit_only:bool=False):
    ...
    required=["core_2023","core_2024","core_2025","core_2026","innerva"]
    print("Saheli CRM - ARCC Full Historical Migration V2")
    ...
    if missing_required:
        print("\nERROR: required ARCC source workbook(s) are missing:")
    ...
    if not sessions: raise RuntimeError("No ARCC sessions parsed from source files")
    ...
    try:
        ...
        print("\n=== DATABASE PREFLIGHT ===")
        print(f"Existing ARCC/Omnia sessions in parsed date range : {len(db_sessions):,}")
        ...
        report=BASE_DIR/f"arcc_migration_{'commit' if commit else 'preview'}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"; log.write(report)
        print("\n=== ACTION SUMMARY ===")
...
def main():
    ap=argparse.ArgumentParser(description="ARCC full historical migration (Core + Innerva)")
