...
def apply_saheli_branding(input_path, output_path):
    ...
    def style_chart(
        chart, title, horizontal=False, *, width=None, height=None,
        show_values=True, show_legend=None, legend_position="b"
    ):
        """Make charts large, readable and consistent in Excel/PDF exports."""
        chart.title = title
        chart.style = 10
        chart.width = width if width is not None else (15.5 if horizontal else 11.5)
        chart.height = height if height is not None else (8.5 if horizontal else 6.8)
        chart.graphical_properties = None

        # Keep axes visible. Horizontal bar charts need the category axis labels
        # explicitly positioned next to the axis, otherwise Excel can hide them.
        try:
        ...
        if show_legend is False:
            chart.legend = None
        elif show_legend is True and chart.legend is not None:
    ...
    style_chart(
        chart, "Current year vs previous published figures",
        width=12.8, height=6.4, show_values=True, show_legend=True, legend_position="b"
    )
    chart.series[0].graphicalProperties.solidFill = magenta
    ...
    donut.holeSize = 58
    style_chart(
        donut, "How verified attendance was constructed",
        width=9.6, height=6.4, show_values=False, show_legend=True, legend_position="b"
    )
    ...
    style_chart(
        gender_chart, "Gender of annual attendees",
        width=10.2, height=6.7, show_values=False, show_legend=True, legend_position="r"
    )
    ...
    style_chart(
        age_chart, "Age profile (% of participants with valid DOB)",
        horizontal=True, width=15.8, height=8.6, show_values=True, show_legend=False
    )
    age_chart.x_axis.numFmt = "0%"
    try:
        age_chart.x_axis.scaling.min = 0
        age_chart.x_axis.scaling.max = 0.30
    except Exception:
        pass
    ...
    while eth_end + 1 <= ws.max_row and ws.cell(eth_end + 1, 1).value not in (None, ""):
        eth_end += 1
    # Build a compact chart helper table using usable ethnicity only.
    helper_row = eth_header
    ws.cell(helper_row, 10, "Ethnicity chart group")
    ws.cell(helper_row, 11, "Participants")
    helper_write = helper_row + 1
    for rr in range(eth_header + 1, eth_end + 1):
        category = ws.cell(rr, 1).value
        value = ws.cell(rr, 2).value
        if category in (None, "", "Not recorded", "Unclear / review"):
            continue
        ws.cell(helper_write, 10, category)
        ws.cell(helper_write, 11, value)
        helper_write += 1
    eth_chart = DoughnutChart()
    eth_chart.add_data(Reference(ws, min_col=11, min_row=helper_row, max_row=helper_write - 1), titles_from_data=True)
    eth_chart.set_categories(Reference(ws, min_col=10, min_row=helper_row + 1, max_row=helper_write - 1))
    eth_chart.holeSize = 62
    style_chart(
        eth_chart, "Ethnicity of participants with usable records",
        width=11.5, height=7.1, show_values=False, show_legend=True, legend_position="r"
    )
    eth_chart.legend.position = "r"
    color_donut(eth_chart, [magenta, pink, yellow, orange, "8E24AA", "6D4C41"])
    ws.add_chart(eth_chart, f"F{eth_header - 1}")
    # Helper cells support the chart only; keep them out of the visible report area.
    ...
    style_chart(
        activity_chart, "Top 10 activities by verified attendance",
        horizontal=True, width=18.5, height=9.5, show_values=True, show_legend=False
    )
    ...
    # Chart only the top 8 reasons. The full table remains visible on the left.
    reason_chart_end = min(reason_end, 14)
    reason_chart = BarChart()
    reason_chart.type = "bar"
    reason_chart.add_data(Reference(ws, min_col=3, min_row=6, max_row=reason_chart_end), titles_from_data=True)
    reason_chart.set_categories(Reference(ws, min_col=1, min_row=7, max_row=reason_chart_end))
    style_chart(
        reason_chart, "Top reasons people join", horizontal=True,
        width=15.5, height=8.2, show_values=True, show_legend=False
    )
    reason_chart.x_axis.numFmt = "0%"
    try:
        reason_chart.x_axis.scaling.min = 0
        reason_chart.x_axis.scaling.max = 0.90
    except Exception:
        pass
    ...
    style_chart(
        heard_chart, "How people heard about Saheli",
        width=11.5, height=7.0, show_values=False, show_legend=True, legend_position="r"
    )
    ...
    style_chart(
        chart, "Bellboat participation by month",
        width=11.5, height=6.2, show_values=True, show_legend=False
    )
    ...
    style_chart(
        chart, "Bellboat and kayaking delivery", horizontal=True,
        width=15.0, height=6.2, show_values=True, show_legend=False
    )
    ...
    chart.holeSize = 58
    style_chart(
        chart, "Bellboat profile gender",
        width=9.5, height=6.0, show_values=False, show_legend=True, legend_position="b"
    )
    chart.legend.position = "b"
    ...
    style_chart(
        chart, "Bellboat age profile", horizontal=True,
        width=14.5, height=7.8, show_values=True, show_legend=False
    )
