#!/usr/bin/env python3
"""
Saheli CRM - Tennis Full Historical Migration V3
...
* 2024 has no source session times: V3 only reuses an unambiguous existing CRM
...
# These cards belong to materially different CRM participants. In the Tennis
# workbook the later rows use the card cell to drive external VLOOKUP profile
# fields, so those profile values are contaminated too. Preserve the actual
# attendee name and resolve the row independently rather than attaching it to
# the wrong FULL participant.
...
# Card 484 is different: the source itself gives a strong alias chain.
# 28-Jul-2026: 484 / Shukriya Aabdulkerim / DOB 1988-07-17 / phone 073835538552
# 04-Aug-2026: 484 / Saffiyah / DOB 1988-07-17 / phone 073835538552
# Live CRM V2 preview independently confirmed card 484 belongs to Shukriya with
# the same DOB. Therefore Saffiyah/Saffiya can be treated as a reviewed alias,
# but only while the live CRM profile still matches this exact evidence.
REVIEWED_CARD_ALIAS_RULES = {
    ("484", "saffiyah"): {
        "crm_name": "shukriya aabdulkerim",
        "crm_dob": date(1988, 7, 17),
        "label": "Saffiyah is a source-reviewed alias of Shukriya Aabdulkerim",
    },
}
...
def apply_reviewed_source_identity_corrections(sessions: list[SourceSession]) -> list[dict[str, Any]]:
    ...
    for s in sessions:
        ...
        for p in s.people:
            # Source spelling variant; preserve the person's displayed alias but make
            # exact-name reconciliation deterministic.
            if normalize_name(p.full_name) == "saffiya":
                audits.append({
                    "Action": "SOURCE_NAME_CANONICALISED",
                    "File": p.source_file, "Sheet": p.source_sheet, "Ref": f"R{p.source_row}",
                    "Date": s.session_date.isoformat(), "Venue": s.venue_name,
                    "Detail": "Saffiya -> Saffiyah (reviewed spelling variant)",
                })
                p.full_name = "Saffiyah"
...
def enrich_source_identity(sessions: list[SourceSession]) -> list[dict[str,Any]]:
    ...
    for card,names in card_names.items():
        ...
        if not unique:
            continue
        norm_names={normalize_name(x) for x in unique}
        if card == "484" and norm_names.issubset({"saffiyah", "shukriya aabdulkerim"}):
            card_canonical[card]="Shukriya Aabdulkerim"
            audits.append({
                "Action":"SOURCE_REVIEWED_CARD_ALIAS", "File":"multiple", "Sheet":"", "Ref":"",
                "Date":"", "Venue":"",
                "Detail":f"card 484 reviewed source aliases={unique}; canonical CRM/source name=Shukriya Aabdulkerim",
            })
        else:
    ...
    for p in people:
        # Normally one-token names are too weak for card propagation. Saffiyah is
        # the one reviewed exception because card 484 is independently linked in
        # this workbook to both Saffiyah and Shukriya with the same DOB.
        allow_name_card_propagation = (
            p.name_key and (len(p.name_key.split())>=2 or p.name_key == "saffiyah")
        )
        if not p.card and allow_name_card_propagation:
            cards=name_cards.get(p.name_key,set())
            if len(cards)==1:
                p.card=next(iter(cards))
                action=("SOURCE_CARD_RESOLVED_BY_REVIEWED_ALIAS" if p.name_key=="saffiyah" else "SOURCE_CARD_RESOLVED_FROM_OTHER_ROW")
                audits.append({"Action":action, "File":p.source_file, "Sheet":p.source_sheet, "Ref":f"R{p.source_row}", "Date":"", "Venue":"", "Detail":f"{p.full_name} -> card {p.card}"})
...
def parse_sources(base_dir: Path):
    ...
    return sessions,a24+a25+reviewed_corrections+identity_actions+dedupe_actions,removed
...
def resolve_member(cur,p,source,participants_by_card,full_by_name,lites_by_key,lite_state,cache,log,dbs=None):
    ...
    if p.card:
        ...
        if existing:
            reviewed_alias = REVIEWED_CARD_ALIAS_RULES.get((card, normalize_name(p.full_name)))
            if reviewed_alias:
                expected_name=reviewed_alias["crm_name"]
                expected_dob=reviewed_alias["crm_dob"]
                if normalize_name(existing.full_name) != expected_name or existing.dob != expected_dob:
                    log.add(
                        "REVIEW_REVIEWED_CARD_ALIAS_CRM_CHANGED", source, p,
                        detail=(
                            f"Reviewed alias rule for card {card} expected CRM name='{expected_name}' DOB={expected_dob}; "
                            f"live CRM is name='{existing.full_name}' DOB={existing.dob or 'NULL'}. Refusing alias reuse."
                        )
                    )
                    return None
                result=(
                    "FULL",str(existing.participant_id),existing.card,existing.full_name or p.full_name,
                    "REUSE_FULL_BY_REVIEWED_CARD_ALIAS"
                )
...
def later_source_time_evidence(sessions: list[SourceSession], source: SourceSession) -> str:
    """Human-readable evidence only; never used to auto-assign a missing 2024 time."""
    same_day = Counter()
    same_venue = Counter()
...
def run_migration(commit=False,audit_only=False,start_filter=None,end_filter=None):
    print("Saheli CRM - Tennis Full Historical Migration V3")
        ...
        for source in sessions:
            ...
            if source.time_quality=="SOURCE_NO_TIME":
                if len(preexisting)==1:
                    ...
                    evidence=later_source_time_evidence(sessions, source)
                    log.add(
                        "REVIEW_2024_SESSION_TIME_MISSING", source,
                        detail=(
                            "2024 source has no time and CRM has no Tennis session for this venue/date; "
                            "refusing to invent time. " + evidence
                        )
                    )
                    continue
                else:
