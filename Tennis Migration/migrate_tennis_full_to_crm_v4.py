#!/usr/bin/env python3
"""
Saheli CRM - Tennis Full Historical Migration V4
...
* 2024 has no source session times: V3 reuses an unambiguous existing CRM session,
  or infers a time only from a strict same-venue + same-weekday pattern with at
  least five later source sessions and exactly one observed slot. Otherwise it blocks.
...
UNKNOWN_TIME_SENTINEL_START = time(0, 0)
UNKNOWN_TIME_SENTINEL_END = time(0, 1)
...
# CRM-verified source-card corrections from the V2 preview.
# These source card assignments point to materially different CRM participants.
# The later source workbook uses the card cell to populate profile/VLOOKUP fields,
# so card-driven DOB/postcode/phone values on these rows are not independent proof.
# Preserve the typed attendee name and resolve that person independently.
REVIEWED_WRONG_SOURCE_CARDS = {
    ("djemilah isakjee", "559"): "CRM card 559 = Zulekha Farooki",
    ("faryaal safdar", "560"): "CRM card 560 = Ghazala Shaheen",
    ("shabana chughtai", "531"): "CRM card 531 = Bian Thiban",
    ("saffiyah", "484"): "CRM card 484 = Shukriya Aabdulkerim",
}


def apply_reviewed_source_identity_corrections(sessions: list[SourceSession]) -> list[dict[str, Any]]:
    audits: list[dict[str, Any]] = []
    for s in sessions:
        keep: list[PersonSource] = []
        for p in s.people:
            card = clean_text(p.card)
            name_key = normalize_name(p.full_name)

            if card and (name_key, card) in REVIEWED_WRONG_SOURCE_CARDS:
                reason = REVIEWED_WRONG_SOURCE_CARDS[(name_key, card)]
                audits.append({
                    "Action": "SOURCE_REVIEWED_WRONG_CARD_CLEARED",
                    "File": p.source_file,
                    "Sheet": p.source_sheet,
                    "Ref": f"R{p.source_row}",
                    "Date": s.session_date.isoformat(),
                    "Venue": s.venue_name,
                    "Detail": (
                        f"source name={p.full_name}; card {card} cleared because {reason}. "
                        "Card-driven DOB/profile fields cleared too; attendee will be resolved independently."
                    ),
                })
                p.card = None
                # These fields are populated from the card lookup on the affected
                # source rows and therefore are not independent identity evidence.
                p.dob = None
                p.postcode = None
                p.phone = None
                p.emergency_name = None
            ...
            # A long numeric value with no usable name is not a defensible Saheli
            # participant identity. Preserve it in the audit but do not create a
            # bogus FULL participant or block the rest of the migration.
            digits = re.sub(r"\D", "", clean_text(p.card))
            if len(digits) >= 7 and not clean_text(p.full_name):
                audits.append({
                    "Action": "SOURCE_UNIDENTIFIABLE_ATTENDANCE_SKIPPED",
                    "File": p.source_file,
                    "Sheet": p.source_sheet,
                    "Ref": f"R{p.source_row}",
                    "Date": s.session_date.isoformat(),
                    "Venue": s.venue_name,
                    "Detail": (
                        f"source identity/card={clean_text(p.raw_card)}; no usable name and no CRM-safe card. "
                        "Attendance row cannot be assigned without inventing a participant."
                    ),
                })
                continue
...
def enrich_source_identity(sessions: list[SourceSession]) -> list[dict[str,Any]]:
    ...
    for p in people:
        # Do not propagate a card from a one-token name alone.
...
def infer_strict_2024_times(sessions: list[SourceSession]) -> list[dict[str, Any]]:
    """Infer a missing 2024 time only when later source evidence is unanimous.

    Safety rule:
      * same canonical venue
      * same weekday
      * at least 5 later (2025/26) dated session groups
      * every one of those later groups has exactly the same start/end slot

    Anything less remains SOURCE_NO_TIME and will be a review blocker unless an
    unambiguous matching CRM session already exists.
    """
    audits: list[dict[str, Any]] = []
    evidence: dict[tuple[str, str], Counter] = defaultdict(Counter)
    for s in sessions:
        if s.source_file != SOURCE_2025 or not s.start_time or not s.end_time:
            continue
        key=(norm_venue(s.venue_name), s.session_date.strftime("%A"))
        evidence[key][(s.start_time,s.end_time)] += 1

    for s in sessions:
        if s.source_file != SOURCE_2024 or s.time_quality != "SOURCE_NO_TIME":
            continue
        key=(norm_venue(s.venue_name), s.session_date.strftime("%A"))
        counter=evidence.get(key, Counter())
        total=sum(counter.values())
        if total >= 5 and len(counter) == 1:
            (st,et),count=next(iter(counter.items()))
            s.start_time=st
            s.end_time=et
            s.time_quality="INFERRED_STRICT_WEEKDAY_PATTERN"
            s.source_notes.append(
                f"2024 source time absent; inferred from {count}/{count} later "
                f"{s.session_date.strftime('%A')} Tennis sessions at the same venue"
            )
            audits.append({
                "Action":"SOURCE_2024_TIME_INFERRED_STRICT_PATTERN",
                "File":s.source_file,
                ...
                "Venue":s.venue_name,
                "Detail":(
                    f"missing 2024 time -> {st.strftime('%H:%M')}-{et.strftime('%H:%M')}; "
                    f"later same-venue {s.session_date.strftime('%A')} evidence is unanimous ({count}/{count})"
                ),
            })
    return audits

def apply_unknown_2024_time_sentinels(sessions: list[SourceSession]) -> list[dict[str, Any]]:
    """Represent genuinely unrecorded 2024 clock times without pretending they are known.

    The CRM schema requires StartTime/EndTime. 00:00-00:01 is therefore used only as a
    migration sentinel meaning SOURCE TIME UNKNOWN. It is intentionally impossible to
    mistake for a normal Tennis delivery slot and is fully documented in Notes/audit.
    """
    audits: list[dict[str, Any]] = []
    for sess in sessions:
        if sess.source_file != SOURCE_2024 or sess.time_quality != "SOURCE_NO_TIME":
            continue
        sess.start_time = UNKNOWN_TIME_SENTINEL_START
        sess.end_time = UNKNOWN_TIME_SENTINEL_END
        sess.time_quality = "SOURCE_TIME_UNKNOWN_SENTINEL"
        sess.source_notes.append(
            "2024 source did not record a session time; 00:00-00:01 is a migration sentinel only, NOT the actual delivery time"
        )
        audits.append({
            "Action": "SOURCE_2024_TIME_UNKNOWN_SENTINEL",
            "File": sess.source_file,
            "Sheet": sess.source_sheet,
            "Ref": sess.source_ref,
            "Date": sess.session_date.isoformat(),
            "Venue": sess.venue_name,
            "Detail": "source time absent; stored as sentinel 00:00-00:01 (NOT actual delivery time; correct later if historical timetable is found)",
        })
    return audits
...
def parse_sources(base_dir: Path):
    ...
    sessions=s24+s25
    strict_time_actions=infer_strict_2024_times(sessions)
    unknown_time_actions=apply_unknown_2024_time_sentinels(sessions)
    reviewed_corrections=apply_reviewed_source_identity_corrections(sessions)
    ...
    return sessions,a24+a25+strict_time_actions+unknown_time_actions+reviewed_corrections+identity_actions+dedupe_actions,removed
...
def create_session(cur,source,template):
    ...
    sentinel_note = (
        "IMPORTANT: 00:00-00:01 is an UNKNOWN-TIME SENTINEL, NOT the actual Tennis delivery time; "
        if source.time_quality == "SOURCE_TIME_UNKNOWN_SENTINEL" else ""
    )
    notes=(f"{MIGRATION_NOTE_PREFIX}; {sentinel_note}source={source.source_file}/{source.source_sheet}/{source.source_ref}; "
...
def resolve_member_from_existing_session_attendance(cur, dbs: DbSession, p: PersonSource):
    """Reuse identity only when this exact CRM session already has exactly one
    attended member with the same normalized source name. This is stronger than
    a global name-only match and can safely disambiguate duplicate Lite names.
    """
    source_name = normalize_name(p.full_name)
    if not source_name:
        return None

    rows = fetch_dicts(cur, """
        SELECT AttendanceMemberKind,ParticipantId,LiteMemberId,MemberDisplayId,
               SaheliCardNumber,MemberName
        FROM dbo.SessionAttendance
        WHERE SessionId=? AND Attended=1
    """, (dbs.session_id,))

    matches = []
    seen = set()
    for r in rows:
        if normalize_name(r.get("MemberName")) != source_name:
            continue

        kind = clean_text(r.get("AttendanceMemberKind")).upper()
        if kind == "FULL" and r.get("ParticipantId") is not None:
            pid = str(int(r["ParticipantId"]))
            ident = ("FULL", pid)
            if ident in seen:
                continue
            seen.add(ident)
            display = clean_text(r.get("SaheliCardNumber")) or clean_text(r.get("MemberDisplayId"))
            matches.append((
                "FULL", pid, display,
                clean_text(r.get("MemberName")) or p.full_name,
                "REUSE_MEMBER_FROM_EXISTING_SESSION_ATTENDANCE",
            ))
        elif kind == "LITE" and r.get("LiteMemberId") is not None:
            lid = str(r["LiteMemberId"])
            ident = ("LITE", lid.lower())
            if ident in seen:
                continue
            seen.add(ident)
            display = clean_text(r.get("MemberDisplayId"))
            matches.append((
                "LITE", lid, display,
                clean_text(r.get("MemberName")) or p.full_name,
                "REUSE_MEMBER_FROM_EXISTING_SESSION_ATTENDANCE",
...
def resolve_member(cur,p,source,participants_by_card,full_by_name,lites_by_key,lite_state,cache,log,dbs=None):
    ...
    if p.card:
        ...
        if existing:
            if p.full_name:
                ...
                if sim < 0.42:
                    log.add(
                        "REVIEW_CARD_NAME_MISMATCH", source, p,
                        detail=(
                            f"card {card} CRM ParticipantID={existing.participant_id} name='{existing.full_name}' "
                            f"DOB={existing.dob or 'NULL'} postcode={existing.postcode or 'NULL'} mobile={existing.mobile or 'NULL'}; "
                            f"source name='{p.full_name}' DOB={p.dob or 'NULL'} postcode={p.postcode or 'NULL'} "
                            f"phone={p.phone or 'NULL'} similarity={sim:.2f}"
                        )
                    )
                    return None
        ...
        if len(re.sub(r"\D","",card))>=7:
            if p.full_name:
                ...
                p.card=None
                return resolve_member(cur,p,source,participants_by_card,full_by_name,lites_by_key,lite_state,cache,log,dbs)
    ...
    if why=="AMBIGUOUS_NAME":
        # The same exact CRM session can provide stronger identity evidence than
        # a global duplicate-name lookup. Only accept a LITE identity that is one
        # of the already-found exact-name Lite candidates.
        if dbs is not None and not dbs.created_in_run:
            existing_session_member=resolve_member_from_existing_session_attendance(cur,dbs,p)
            candidate_ids={x.lite_id.lower() for x in cands}
            if (existing_session_member and existing_session_member[0]=="LITE"
                    and existing_session_member[1].lower() in candidate_ids):
                result=(
                    existing_session_member[0], existing_session_member[1], existing_session_member[2],
                    existing_session_member[3], "REUSE_LITE_BY_EXISTING_SESSION_ATTENDANCE"
                )
                cache[cache_key]=result
                return result

        bits=" | ".join(
            f"{x.membership_id}[name={x.full_name},dob={x.dob or '-'},postcode={x.postcode or '-'},phone={x.phone or '-'}]"
            for x in cands
        )
        # When the source contains only a name and multiple real CRM people share it,
        # attaching the attendance to either person would corrupt identity history.
        # Do not manufacture a third duplicate Lite either. Preserve the unresolved
        # source attendance in the audit and leave it for later manual reconciliation.
        if not p.dob and not p.postcode and not p.phone:
            log.add(
                "SOURCE_AMBIGUOUS_LITE_ATTENDANCE_SKIPPED", source, p,
                detail=(f"source name={p.full_name}; source has no DOB/postcode/phone; "
                        f"multiple exact-name Lite candidates: {bits}; not attached to an arbitrary person")
            )
            return None

        log.add(
            "REVIEW_AMBIGUOUS_LITE_NAME", source, p,
            detail=(f"source name={p.full_name}; DOB={p.dob or 'NULL'}; postcode={p.postcode or 'NULL'}; "
                    f"phone={p.phone or 'NULL'}; candidates: {bits}")
        ); return None
...
def print_source_summary(sessions,source_actions,dupes):
    ...
    inferred_2024=[s for s in dated_2024 if s.time_quality=="INFERRED_STRICT_WEEKDAY_PATTERN"]
    sentinel_2024=[s for s in dated_2024 if s.time_quality=="SOURCE_TIME_UNKNOWN_SENTINEL"]
    unresolved_2024=[s for s in dated_2024 if s.time_quality=="SOURCE_NO_TIME"]
    ...
    print(f"  2024 session groups: {len(dated_2024)}")
    print(f"    strict time inferred: {len(inferred_2024)}")
    print(f"    time unknown sentinel (00:00-00:01): {len(sentinel_2024)}")
    print(f"    time still unresolved: {len(unresolved_2024)}")
    print(f"  2025/26 timed session groups: {len(later)}")
    print(f"Clean migratable attendance rows: {clean_att}")
    unidentifiable=sum(1 for r in source_actions if r.get("Action")=="SOURCE_UNIDENTIFIABLE_ATTENDANCE_SKIPPED")
    if unidentifiable:
        print(f"Unassignable source attendance skipped with audit: {unidentifiable}")
...
def later_source_time_evidence(sessions: list[SourceSession], source: SourceSession) -> str:
    """Human-readable evidence for 2024 sessions still unresolved after strict inference."""
    same_day = Counter()
    same_venue = Counter()
    weekday = source.session_date.strftime("%A")
    for candidate in sessions:
        if candidate.time_quality == "SOURCE_NO_TIME" or not candidate.start_time or not candidate.end_time:
            continue
        if norm_venue(candidate.venue_name) != norm_venue(source.venue_name):
            continue
        same_venue[(candidate.start_time, candidate.end_time)] += 1
        if candidate.session_date.strftime("%A") == weekday:
            same_day[(candidate.start_time, candidate.end_time)] += 1

    def fmt(counter: Counter) -> str:
        if not counter:
            return "none"
        return ", ".join(
            f"{st.strftime('%H:%M')}-{et.strftime('%H:%M')} x{count}"
            for (st, et), count in sorted(counter.items(), key=lambda z: (-z[1], z[0][0], z[0][1]))
        )

    return (
        f"later source {weekday} slots at venue: {fmt(same_day)}; "
        f"all later venue slots: {fmt(same_venue)}"
    )


def write_review_csv(log: MigrationLog, path: Path) -> None:
    fields=[
        "Action","Date","Venue","Activity","SourceFile","SourceSheet","SourceSessionRef",
        "SessionId","SourceCard","SourceRawIdentity","SourceName","MemberRef","Detail"
    ]
    rows=[r for r in log.rows if str(r.get("Action", "")).startswith("REVIEW_")]
    with path.open("w", newline="", encoding="utf-8-sig") as f:
...
def print_review_details(log: MigrationLog) -> None:
    rows=[r for r in log.rows if str(r.get("Action", "")).startswith("REVIEW_")]
    if not rows:
        return
    print("\nReview details (grouped):")
    grouped=defaultdict(list)
    for row in rows:
        key=(
            row.get("Action", ""), row.get("Date", ""), row.get("Venue", ""),
            row.get("SourceCard", ""), row.get("SourceName", ""), row.get("Detail", "")
        )
        grouped[key].append(row)
    for key, items in sorted(grouped.items(), key=lambda kv: (kv[0][0],kv[0][1],kv[0][2],kv[0][3],kv[0][4])):
        action,d,venue,card,name,detail=key
        who = f" | card={card or '-'} | name={name or '-'}" if card or name else ""
        print(f"  {len(items):3d} row(s) | {action} | {d or '-'} | {venue or '-'}{who} | {detail}")
...
def run_migration(commit=False,audit_only=False,start_filter=None,end_filter=None):
    print("Saheli CRM - Tennis Full Historical Migration V4")
    ...
    try:
        ...
        for source in sessions:
            ...
            if source.time_quality=="SOURCE_NO_TIME":
                # Defensive fallback only: parse_sources normally converts these to
                # SOURCE_TIME_UNKNOWN_SENTINEL before DB work.
                if len(preexisting)==1:
                    ...
                    log.add("REVIEW_2024_SESSION_TIME_MISSING",source,detail="Unexpected unresolved 2024 source time reached DB phase")
                    continue
            elif source.time_quality=="SOURCE_TIME_UNKNOWN_SENTINEL":
                sentinel=[x for x in preexisting if x.start_time==UNKNOWN_TIME_SENTINEL_START and x.end_time==UNKNOWN_TIME_SENTINEL_END]
                real=[x for x in preexisting if x not in sentinel]
                if sentinel:
                    dbs=sorted(sentinel,key=lambda x:(-x.attendance_count,x.session_id))[0]
                    log.add("REUSE_2024_UNKNOWN_TIME_SENTINEL_SESSION",source,detail=f"SessionId={dbs.session_id}; 00:00-00:01 is sentinel only",session_id=dbs.session_id)
                elif len(real)==1:
                    dbs=real[0]
                    log.add("REUSE_2024_SESSION_BY_UNAMBIGUOUS_DATE",source,detail=f"SessionId={dbs.session_id}; CRM time={dbs.start_time}-{dbs.end_time}; source time was not recorded",session_id=dbs.session_id)
                elif len(real)>1:
                    detail=", ".join(f"{x.session_id}:{x.start_time}-{x.end_time}" for x in sorted(real,key=lambda z:z.session_id))
                    log.add("REVIEW_2024_SESSION_AMBIGUOUS",source,detail=f"source time unknown and multiple real CRM Tennis sessions exist on venue/date: {detail}")
                    continue
                else:
                    ...
                    if not template:
                        log.add("REVIEW_NO_TENNIS_TEMPLATE",source,detail="No existing Tennis session with Category metadata found to template unknown-time historical session")
                        continue
                    try:
                        dbs=create_session(cur,source,template); db_sessions.append(dbs)
                        log.add("CREATE_2024_UNKNOWN_TIME_SENTINEL_SESSION",source,detail=f"SessionId={dbs.session_id}; sentinel=00:00-00:01; template={template.get('SessionId')}",session_id=dbs.session_id)
                    except Exception as exc:
                ...
                if dbs:
                    ...
                    # Tennis identity is venue + date + time slot. A different start time on the
                    # same date is a separate legitimate Tennis session and is allowed to coexist.
                    # Only multiple PRE-EXISTING sessions sharing the source start time are ambiguous.
                    same_start=[x for x in preexisting if x.start_time==source.start_time]
                    if len(same_start)>1:
                        detail=", ".join(
                            f"{x.session_id}:{x.start_time}-{x.end_time}:att={x.attendance_count}:cancelled={int(x.is_cancelled)}"
                            for x in sorted(same_start,key=lambda z:z.session_id)
                        )
                        log.add(
                            "REVIEW_SESSION_SAME_START_AMBIGUOUS", source,
                            detail=f"source={source.start_time}-{source.end_time}; multiple CRM same-start candidates={detail}"
                        )
                        continue
                    different_slots=[x for x in preexisting if x.start_time!=source.start_time]
                    if different_slots:
                        detail=", ".join(
                            f"{x.session_id}:{x.start_time}-{x.end_time}"
                            for x in sorted(different_slots,key=lambda z:z.session_id)
                        )
                        log.add(
                            "EXISTING_OTHER_TENNIS_SLOT_SAME_DATE", source,
                            detail=(f"source slot={source.start_time}-{source.end_time}; other CRM slot(s)={detail}; "
                                    "source slot handled independently")
                        )
                    template=template_cache.get(source.venue_name)
                    if template is None:
                        template=load_tennis_template(cur,source.venue_name); template_cache[source.venue_name]=template
            ...
            if source.is_cancelled:
                if dbs.attendance_count>0 and not dbs.created_in_run:
                    log.add(
                        "SOURCE_CANCELLED_CONFLICT_EXISTING_ATTENDANCE_PRESERVED",
                        source,
                        detail=(
                            f"SessionId={dbs.session_id} has {dbs.attendance_count} attended CRM rows; "
                            f"source marker='{source.cancel_reason or 'cancelled'}'. "
                            "Existing CRM attendance and cancellation status preserved."
                        ),
            ...
            for p in source.people:
                resolved=resolve_member(
                    cur,p,source,participants_by_card,full_by_name,lites_by_key,
                    lite_state,member_cache,log,dbs
                )
        ...
        review_path=BASE_DIR/f"tennis_migration_review_{stamp}.csv"; write_review_csv(log,review_path)
