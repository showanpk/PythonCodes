...
#
# Compares against SQL Server tables and:
#   - INSERTS missing rows
#   - UPDATES existing rows if any compared field is different
...
DEFAULT_CREATED_BY_USER_ID = None  # e.g. "system-import"

# Toggle if you want row-by-row debug on bulk insert failure
DEBUG_ROW_FALLBACK = True

# Toggle if you want detailed changed column printing during updates
DEBUG_SHOW_CHANGED_COLUMNS = False
...
# =========================
# NEW: COMPARE + UPDATE HELPERS
# =========================
def normalize_for_compare(v):
    """
    Normalize Python/SQL values so comparisons are reliable.
    Avoid false mismatches (e.g., 1 vs 1.0, datetime vs date, whitespace).
    """
    if v is None:
        return None
    ...
    # Datetime -> strip microseconds for safe compare
    ...
    # strings -> trim and blank as None
    if isinstance(v, str):
        s = v.strip()
        return s if s != "" else None
...
def sql_existing_rows(cursor, table_name, select_cols, key_cols):
    """
    Returns dict:
      { (key_tuple): {col: value, ...} }
    """
...
def update_rows(cursor, table_name, rows, key_cols, update_cols):
    """
    Row-by-row update (WHERE on key cols, SET non-key cols)
    """
    if not rows:
        return 0
    ...
    sql = f"UPDATE dbo.[{table_name}] SET {set_sql} WHERE {where_sql}"

    updated = 0
    cursor.fast_executemany = False  # safer for updates
    for r in rows:
        params = [r.get(c) for c in update_cols] + [r.get(k) for k in key_cols]
        cursor.execute(sql, params)
        updated += 1
    return updated


def upsert_by_compare(cursor, table_name, rows, key_cols, all_insert_cols):
    """
    Sync logic:
      - Insert missing rows
      - Update existing rows if any non-key column mismatched
      - Skip unchanged rows
    """
    if not rows:
        print(f"[{table_name}] Source: 0 | Unique: 0 | New: 0 | Updated: 0 | Unchanged: 0")
        return {"inserted": 0, "updated": 0, "unchanged": 0}

    unique_rows = _dedupe_row_dicts(rows, key_cols)

    existing_map = sql_existing_rows(cursor, table_name, all_insert_cols, key_cols)

    to_insert = []
    to_update = []
    unchanged = 0

    non_key_cols = [c for c in all_insert_cols if c not in key_cols]

    for src in unique_rows:
        key = tuple(src.get(k) for k in key_cols)
        ...
        for c in non_key_cols:
            if values_different(src.get(c), db_row.get(c)):
                changed_cols.append(c)

        if changed_cols:
            if DEBUG_SHOW_CHANGED_COLUMNS:
                print(f"[DEBUG] {table_name} key={key} changed columns: {changed_cols}")
            to_update.append(src)
        else:
            unchanged += 1

    inserted = insert_rows(cursor, table_name, to_insert, all_insert_cols)
    updated = update_rows(cursor, table_name, to_update, key_cols, non_key_cols)

    print(
        f"[{table_name}] Source: {len(rows)} | Unique: {len(unique_rows)} | "
        f"New: {len(to_insert)} | Updated: {updated} | Unchanged: {unchanged}"
    )
...
def build_reg_site_lookup(df_reg: pd.DataFrame):
    """
    Build {SaheliCardNumber(int): Site(str)} from REG file.
    Uses REG column Site or Site:
    """
    m, col_saheli = ensure_reg_core_columns(df_reg)
    col_site = pick_col(m, "Site", "Site:")

    if not col_site:
        print("\n[DEBUG] REG normalized headers (Site not found):")
        for k, cols in sorted(m.items()):
            print(f"  {k}: {cols}")
        return {}

    df = df_reg.copy()
    df["_SaheliKey"] = df[col_saheli].apply(to_saheli_key)
    df["_SiteVal"] = df[col_site].apply(to_str)

    df = df[df["_SaheliKey"].notna()].copy()
    df = df[df["_SiteVal"].notna()].copy()

    # Remove junk placeholders
    bad_values = {"n/a", "na", "none", "null", "-"}
    df = df[~df["_SiteVal"].str.strip().str.lower().isin(bad_values)].copy()

    # Keep first site per Saheli
    df = dedupe_by_key(df, ["_SaheliKey"])

    lookup = {}
    for _, r in df.iterrows():
        k = to_int(r["_SaheliKey"])
        if k is None:
            continue
        lookup[k] = to_str(r["_SiteVal"])
...
def build_participants_from_reg(df_reg: pd.DataFrame):
    ...
    for _, r in df.iterrows():
        rows.append({
            ...
            "GenderSameAsBirth": to_str(r[col_gender_same]) if col_gender_same else None,
            ...
            "LivingAlone": to_str(r[col_living_alone]) if col_living_alone else None,
            "CaringResponsibilities": to_str(r[col_caring]) if col_caring else None,
...
def build_assessment_master_rows(dfh, m):
    ...
    for _, r in dfh.iterrows():
        rows.append({
            ...
            "CreatedAtUtc": parse_datetime(r["_AssessmentDate"]) if r["_AssessmentDate"] else None,
            "CreatedByUserId": DEFAULT_CREATED_BY_USER_ID,
        })
    return rows


def build_assessments_rows(dfh, m, reg_site_lookup=None):
    """
    Site priority:
      1) Health file Site column (if exists and not blank)
      2) REG file site lookup by SaheliCardNumber
    """
    reg_site_lookup = reg_site_lookup or {}
    ...
    rows = []
    for _, r in dfh.iterrows():
        saheli_num = to_int(r["_SaheliKey"])
        health_site = to_str(r[c_site]) if c_site else None
        reg_site = reg_site_lookup.get(saheli_num)
        final_site = health_site if health_site else reg_site

        rows.append({
            "SaheliCardNumber": saheli_num,
            "AssessmentNumber": to_int(r["_AssessmentNumber"]),
            "AssessmentDate": r["_AssessmentDate"],
...
def build_community_conf_rows(dfh, m):
    ...
    for _, r in dfh.iterrows():
        rows.append({
            ...
            "AssessmentNumber": to_int(r["_AssessmentNumber"]),
            "AssessmentDate": r["_AssessmentDate"],
            "ConfidenceToJoin": to_float(r[c_conf]) if c_conf else None,
            "NumberOfHobbies": to_float(r[c_hobbies]) if c_hobbies else None,
            "CommunityInvolvement": to_float(r[c_comm]) if c_comm else None,
            "ServiceAwareness": to_float(r[c_serv]) if c_serv else None,
        })
    return rows
...
def build_health_screening_rows(dfh, m):
    ...
    for _, r in dfh.iterrows():
        ...
        rows.append({
            ...
            "HeartRateBpm": to_float(r[c_hr]) if c_hr else None,
            "AtrialFibrillationResult": to_first_int_from_text(r[c_af]) if c_af else None,
            "HeartAge": to_float(r[c_ha]) if c_ha else None,
            ...
            "SelfManagementScore": to_float(r[c_self_mgmt]) if c_self_mgmt else None,
        })
    return rows
...
def build_lifestyle_rows(dfh, m):
    ...
    for _, r in dfh.iterrows():
        rows.append({
            ...
            "AssessmentNumber": to_int(r["_AssessmentNumber"]),
            "AssessmentDate": r["_AssessmentDate"],
            "Nourishment": to_float(r[c_n]) if c_n else None,
            "Movement": to_float(r[c_m]) if c_m else None,
            "Connectedness": to_float(r[c_c]) if c_c else None,
            "SleepQuality": to_float(r[c_s]) if c_s else None,
            "HappySelf": to_float(r[c_h]) if c_h else None,
            "Resilience": to_float(r[c_r]) if c_r else None,
            "GreenBlueSpace": to_float(r[c_g]) if c_g else None,
            "ScreenTime": to_float(r[c_st]) if c_st else None,
            "SubstanceUse": to_float(r[c_sub]) if c_sub else None,
            "Purpose": to_float(r[c_p]) if c_p else None,
...
def build_physical_activity_rows(dfh, m):
    ...
    for _, r in dfh.iterrows():
        rows.append({
            ...
            "AssessmentNumber": to_int(r["_AssessmentNumber"]),
            "AssessmentDate": r["_AssessmentDate"],
            "ActiveDaysPerWeek": to_float(r[c_days]) if c_days else None,
...
def build_social_isolation_rows(dfh, m):
    ...
    for _, r in dfh.iterrows():
        rows.append({
            ...
            "AssessmentNumber": to_int(r["_AssessmentNumber"]),
            "AssessmentDate": r["_AssessmentDate"],
            "LackCompanionship": to_float(r[c_lack]) if c_lack else None,
            "FeelLeftOut": to_float(r[c_left]) if c_left else None,
            "FeelIsolated": to_float(r[c_iso]) if c_iso else None,
...
def build_wemwbs_rows(dfh, m):
    ...
    for _, r in dfh.iterrows():
        ...
        for out_col, src_col in cols.items():
            row[out_col] = to_float(r[src_col]) if src_col else None
        rows.append(row)
    return rows


# =========================
# LEGACY INSERT-ONLY (kept, not used now)
# =========================
...
def main():
    ...
    # Build site lookup from REG
    reg_site_lookup = build_reg_site_lookup(df_reg)
    print(f"REG Site lookup rows: {len(reg_site_lookup)}")
    ...
    rows_assessments = build_assessments_rows(dfh, hm, reg_site_lookup=reg_site_lookup)
    ...
    print("\n=== SQL CONNECT & UPSERT (COMPARE + UPDATE) ===")
    ...
    try:
        ...
        # 2) ParticipantEmergencyContacts (after Participants so ParticipantID map exists)
        ...
        upsert_by_compare(
            ...
            emergency_rows,
            key_cols=["SaheliCardNumber"],
            all_insert_cols=["SaheliCardNumber", "ContactName", "ContactNumber", "Relationship", "ParticipantID"],
        )
        ...
        upsert_by_compare(
            ...
            all_insert_cols=["SaheliCardNumber", "AssessmentNumber", "AssessmentDate", "CreatedAtUtc", "CreatedByUserId"],
        )
        ...
        upsert_by_compare(
            ...
            rows_aims,
            key_cols=["SaheliCardNumber", "AssessmentNumber"],
            all_insert_cols=["SaheliCardNumber", "AssessmentNumber", "AssessmentDate", "AimsGoals", "AimsDescription"],
        )
        ...
        upsert_by_compare(
            ...
            rows_barriers,
            key_cols=["SaheliCardNumber", "AssessmentNumber"],
            all_insert_cols=["SaheliCardNumber", "AssessmentNumber", "AssessmentDate", "Barriers", "BarrierComments"],
        )
        ...
        conn.commit()
        print("\n✅ SUCCESS: Sync complete (inserted missing rows, updated mismatches, skipped unchanged rows).")
