...
#
# FIXED OUTPUT:
#   - Final Excel is written with 2-row grouped headers:
#       Row 1: 1st Assessment / 2nd Assessment / ...
#       Row 2: Weight (KG): / Height (CM): / ...
#   - So it looks like your expected layout
#
# Install:
...
# (Optional debug/helper file - flat internal columns)
FINAL_OUTPUT_FLAT_DEBUG_FILE = r"C:\Users\shonk\source\PythonCodes\New folder\Saheli_Master_Wide_Output_FLAT_DEBUG.xlsx"
...
def clean_subheader_label(label: str) -> str:
    """
    Remove leading/trailing spaces for display subheaders.
    Keep punctuation because user wants original labels.
    """
    if label is None:
        return ""
    s = str(label).replace("\r", " ").replace("\n", " ")
    s = re.sub(r"\s+", " ", s).strip()

    # Display fixes requested/expected
    if s == "WEMBS":
        s = "WEMWBS"  # display label
    if s == "SOCIAL ISOLATION":
        s = "SOCIAL"  # display label as in expected
    return s


def make_display_assessment_header(block: str, fld: str) -> tuple[str, str]:
    """
    Convert flat internal column names to grouped 2-row header:
      ('1st Assessment', 'Weight (KG):')
      ('1st Assessment', '2nd Assessment')  # for the date column
    """
    fld_clean = clean_subheader_label(fld)
    # Block date column should show "1st Assessment" in row2 (your expected)
    if fld_clean == block:
        return (block, block)
    return (block, fld_clean)
...
# =========================
# STEP 3: BUILD FINAL WIDE (flat internal)
# =========================
def create_final_wide_df(df_reg_clean: pd.DataFrame, df_health_prepared: pd.DataFrame) -> pd.DataFrame:
    ...
    # Registration base (one row per Saheli, REG is primary)
    ...
    # Keep one registration row per Saheli (first/earliest)
    ...
    def health_source_col(label):
        # Comments mapping (same source label repeated in form)
        if label == " Comments:":
    ...
    for n in range(1, MAX_ASSESSMENTS + 1):
        ...
        # Put assessment date in block column (e.g. "1st Assessment")
        tmp[block] = h_n["_CompletionDate"].values

        # Add all assessment fields
        ...
        # Rename to flat-wide internal columns
        rename_map = {}
        for c in tmp.columns:
            ...
            if c == block:
                ...
                rename_map[c] = f"{block}  {clean_subheader_label(c)}"  # keep internal uniqueness
    ...
    def assessment_sort_key(c):
        ...
        if m:
            ...
            suffix = m.group(3) or ""
            # Keep block date column first in each assessment block
            if suffix.strip() == "":
                return (0, block_n, "", 0)
            return (0, block_n, suffix.lower(), 1)
        return (1, 999, str(c).lower(), 9)
...
# =========================
# FINAL OUTPUT WRITER (GROUPED HEADERS)
# =========================
def write_final_grouped_excel(flat_df: pd.DataFrame, out_path: str):
    """
    Writes the final output with 2-row grouped headers for assessment columns:
      Row 1: 1st Assessment | 1st Assessment | ... | 2nd Assessment | ...
      Row 2: 1st Assessment | Weight (KG):   | ... | 2nd Assessment | ...
    Registration columns are kept ungrouped (Row1 blank, Row2 actual names).
    """
    # Keep display copy (don't mutate flat_df)
    df = flat_df.copy()

    # Build MultiIndex-like display columns manually
    top_headers = []
    sub_headers = []
    original_cols = list(df.columns)

    for col in original_cols:
        m = re.match(r"^(\d+)(st|nd|rd|th)\sAssessment(?:\s{2,}(.*))?$", str(col))
        if m:
            block = f"{m.group(1)}{m.group(2)} Assessment"
            suffix = (m.group(3) or "").strip()

            if suffix == "":
                # Date column for this assessment block
                top, sub = make_display_assessment_header(block, block)
            else:
                top, sub = make_display_assessment_header(block, suffix)

            top_headers.append(top)
            sub_headers.append(sub)
        else:
            top_headers.append("")  # registration/general columns ungrouped
            sub_headers.append(str(col).strip())

    # Write raw data first (without header), then inject two header rows
    out = Path(out_path)
    out.parent.mkdir(parents=True, exist_ok=True)

    with pd.ExcelWriter(out, engine="openpyxl", datetime_format="dd/mm/yyyy", date_format="dd/mm/yyyy") as writer:
        # Write data starting row 3 (index 2)
        df.to_excel(writer, index=False, header=False, startrow=2, sheet_name="Sheet1")

    # Format and write grouped headers with openpyxl
    ...
    # Merge top header cells for assessment blocks
    start_col = None
    current_label = None
    for col_idx, top in enumerate(top_headers, start=1):
        if top != current_label:
            # close previous
            if current_label and start_col is not None and col_idx - 1 > start_col:
                ws.merge_cells(start_row=1, start_column=start_col, end_row=1, end_column=col_idx - 1)
            current_label = top
            start_col = col_idx

    # close final
    if current_label and start_col is not None and len(top_headers) > start_col:
        ws.merge_cells(start_row=1, start_column=start_col, end_row=1, end_column=len(top_headers))
    ...
    # Determine which assessment block each column belongs to for color
    for col_idx, top in enumerate(top_headers, start=1):
        cell_top = ws.cell(row=1, column=col_idx)
        cell_sub = ws.cell(row=2, column=col_idx)

        if top == "":
            # Registration columns
            cell_top.fill = dark_fill
            cell_sub.fill = dark_fill
            cell_top.font = white_font
            cell_sub.font = white_font
        else:
            # Alternate colors by assessment number
            num_match = re.match(r"^(\d+)", top)
            n = int(num_match.group(1)) if num_match else 1
            fill = purple_fill if n % 2 == 1 else blue_fill
        ...
        cell_sub.alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)

    # Freeze panes below 2 header rows
    ws.freeze_panes = "A3"

    # Date formatting for data area
    for col_idx, sub in enumerate(sub_headers, start=1):
        sub_norm = normalize_header(sub)
        if sub_norm in {
            normalize_header("Registration Date"),
            normalize_header(" Date of Birth:"),
        } or "assessment" in normalize_header(sub) or sub_norm == normalize_header("Date of next review appointment:"):
            for r in range(3, ws.max_row + 1):
                ws.cell(row=r, column=col_idx).number_format = "dd/mm/yyyy"

    # Widths (simple auto-ish)
    for col_idx, sub in enumerate(sub_headers, start=1):
        text = str(sub)
        # Keep first columns wider and assessment labels moderately wide
        if col_idx <= 12:
            width = min(max(len(text) + 4, 14), 28)
        else:
            width = min(max(len(text) * 0.9, 12), 45)
    ...
    # Bold data header-like first row? no
...
def main():
    ...
    # Optional: save flat debug version (useful for SQL script or troubleshooting)
    write_excel(final_df, FINAL_OUTPUT_FLAT_DEBUG_FILE)
    print("Final flat debug saved:", FINAL_OUTPUT_FLAT_DEBUG_FILE)

    # Save grouped visual output (this is your main file)
    write_final_grouped_excel(final_df, FINAL_OUTPUT_FILE)
    print("Final grouped output saved:", FINAL_OUTPUT_FILE)
    ...
    # Quick preview (flat internal names)
    preview_cols = [
        c for c in [
