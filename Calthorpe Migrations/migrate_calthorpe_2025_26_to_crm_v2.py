...
This V2 script also resolves the previously unmatched records:

    - Missing card numbers are created as FULL participants.
    - One exact FULL-name match is linked to the existing participant.
    - A cardless name matching multiple FULL participants is created as a
      separate deterministic LiteMember rather than being linked incorrectly.

Keep COMMIT_CHANGES = False for the first V2 run.
Set COMMIT_CHANGES = True only after the V2 preview shows no unresolved
attendance records.
"""
...
# V2 first run: all participant-resolution options are enabled,
# but the whole transaction is still rolled back.
COMMIT_CHANGES = True

CREATE_MISSING_LITE_MEMBERS = True
CREATE_MISSING_FULL_PARTICIPANTS = True

# A cardless source record with exactly one matching FULL name is linked
# to that existing participant.
MATCH_UNIQUE_FULL_NAME = True

# When a cardless source name matches several FULL participants, do not
# guess which ParticipantId is correct. Create a deterministic LiteMember
# for that source identity instead.
CREATE_AMBIGUOUS_NAME_AS_LITE = True
...
def create_full_participant(
    cursor: pyodbc.Cursor,
    row: dict[str, Any],
) -> dict[str, Any]:
    """
    Create a missing FULL participant when the historical register contains
    a Saheli Card Number that is genuinely absent from dbo.Participants.

    dbo.Participants only requires SaheliCardNumber. The source name,
    registration date, site and migration note are included when available.
    """
    card = normalise_card(row.get("SaheliCardNumber"))

    if not card:
        raise ValueError(
            "Cannot create a FULL participant without SaheliCardNumber: "
            f"{row.get('SourceParticipantKey')}"
        )

    source_name = clean_text(row.get("SourceName"))
    full_name = source_name or f"Historical participant card {card}"

    registration_date = parse_optional_date(
        row.get("FirstAttendanceDate")
    )

    notes = (
        "Created from historical Calthorpe 2025/26 register migration. "
        f"Source key: {clean_text(row.get('SourceParticipantKey'))}."
    )
...
def main() -> int:
    ...
    try:
            ...
            if kind == "FULL":
                ...
                if card not in participants_by_card:
                    if CREATE_MISSING_FULL_PARTICIPANTS:
                        preflight["PREFLIGHT_FULL_TO_CREATE"] += 1
                        add_audit(
                            audit,
                            "PREFLIGHT",
                            source_key,
                            "PREFLIGHT_FULL_TO_CREATE",
                            "",
                            (
                                f"Card {card!r} is not in dbo.Participants "
                                "and will be created inside the transaction."
                            ),
                        )
                    else:
        ...
        for row in participant_rows:
            ...
            if expected_kind == "FULL":
                ...
                if full:
                ...
                elif CREATE_MISSING_FULL_PARTICIPANTS:
                    member = create_full_participant(
                        cursor,
                        row,
                    )
                    action = "CREATED_FULL"

                    participants_by_card[
                        member["card"]
                    ] = member

                    created_name_key = normalise_name(
                        member.get("name")
                    )
                    ...
                    details = (
                        f"Created FULL participant for missing card "
                        f"{card!r} inside the current SQL transaction."
                    )
                ...
                if existing_lite:
                    ...
                    if len(lite_candidates) == 1:
                        ...
                        and MATCH_UNIQUE_FULL_NAME
                    ):
                        member = {
                            "kind": "FULL",
                            **full_candidates[0],
                        }
                        action = "MATCHED_FULL_BY_UNIQUE_NAME"
                        details = (
                            "The cardless source record was linked to "
                            "the only FULL participant with the same "
                            "normalised name."
                        )

                    elif (
                        len(full_candidates) > 1
                        and CREATE_AMBIGUOUS_NAME_AS_LITE
                        and CREATE_MISSING_LITE_MEMBERS
                    ):
                        ...
                        action = "CREATED_LITE_AMBIGUOUS_FULL_NAME"
                        ...
                        details = (
                            ...
                            "have the same name. A separate deterministic "
                            "LiteMember was created rather than linking "
                            "attendance to the wrong ParticipantId."
                        )
                        ...
                        details = (
                            ...
                            "name, but automatic unique-name matching "
                            "is disabled."
                        )
    ...
    if not COMMIT_CHANGES:
        ...
        print(
            "Review the audit CSV. Commit only when "
            "UNMATCHED_PARTICIPANT and UNMATCHED_CARD are both zero."
        )
