#!/usr/bin/env python3
"""
Saheli CRM - Bike Giveaway Historical Migration V1.5
==================================================
...
V1.5 fixes:
- Keeps all V1.4 safety rules and source-row idempotency.
- Adds one final narrowly-scoped reviewed mapping for Iffat Shaheen source row 2.
- The mapping targets existing FULL card 533 and validates source row, giveaway,
  normalized name, Wellbeing ID, email, gender, target name, gender, DOB and
  postcode before reuse.
- If any expected evidence changes, the row becomes REVIEW_* and commit is refused.
...
# ---------------------------------------------------------------------------
# MANUALLY REVIEWED IDENTITY OVERRIDES
# ---------------------------------------------------------------------------
#
# These mappings were added after V1/V1.1/V1.2 previews and read-only CRM
# diagnostics. They ONLY select an existing FULL participant for attendance.
# They do not update Participants or copy source Wellbeing/email values into CRM.
#
REVIEWED_FULL_OVERRIDES = {
    (1, 2, "iffat shaheen"): {
        "saheli_card": "533",
        "expected_wellbeing": "THE1769791952260",
        "expected_email": "asififfat42@gmail.com",
        "expected_gender": "female",
        "expected_target_dob": "1982-02-15",
        "expected_target_postcode": "B8 2QW",
        "reason": (
            "Reviewed final blocker. FULL 533 is the established fully-profiled "
            "female Iffat Shaheen record, registered 2026-02-11, six days before "
            "Giveaway 1. Competing LITE-828 is a sparse Lite record created on "
            "2026-09-10 from historical tennis attendance and has no DOB/postcode/"
            "phone/email. No exact source Wellbeing-ID or email match exists elsewhere."
        ),
    },
    (1, 12, "hayat ali"): {
        "saheli_card": "1247",
        "expected_wellbeing": "THE1769785179930",
        "expected_email": "hayatali1963@icloud.com",
        "expected_gender": "male",
        "reason": (
            "Reviewed exact-name/gender match. FULL 1247 is the established Hayat Ali "
            "record (DOB 1963-07-12). The source email is not used for profile matching "
            "because live CRM stores that exact email against another participant."
        ),
    },
    (1, 19, "yasmeen akhtar"): {
        "saheli_card": "1135",
        "expected_wellbeing": "MCR022558122",
        "expected_email": "yasmeen-akhtar@hotmail.co.uk",
        "expected_gender": "female",
        "reason": (
            "Reviewed exact-name/gender match. FULL 1135 is the established active "
            "Yasmeen Akhtar record with substantial prior CRM attendance, including "
            "activity on the Giveaway 1 date; competing FULL 357 has no attendance."
        ),
    },
    (1, 20, "shamim akhtar"): {
        "saheli_card": "1460",
        "expected_wellbeing": "MCR022496285",
        "expected_email": "shamim-akhtar@hotmail.co.uk",
        "expected_gender": "female",
        "reason": (
            "Reviewed exact-name/gender match. FULL 1460 is the only Shamim Akhtar "
            "candidate with a personal email closely matching the source email stem."
        ),
    },
    (5, 138, "noreen akhtar"): {
        "saheli_card": "1626",
        "expected_wellbeing": "THE1774433555904",
        "expected_email": "",
        "expected_gender": "female",
        "reason": (
            "Reviewed exact-name/gender match. FULL 1626 is the current fully-profiled "
            "female Noreen Akhtar record active in 2026; no exact source Wellbeing-ID "
            "match was found elsewhere in CRM."
        ),
    },
    (5, 140, "shahida ali"): {
        "saheli_card": "1067",
        ...
        "expected_gender": "female",
        "reason": (
            "Reviewed exact-name/gender match. FULL 1067 is the established fully-"
            "profiled female Shahida Ali record; competing LITE-441 is sparse."
        ),
    },
}
...
def source_attendance_status(cur, session: DbSession, p: Recipient, log: MigrationLog) -> str:
    """
    Idempotency guard based on the immutable source-row marker written by this migration.

    Returns:
      "NONE"    -> no prior migration attendance for this source row
      "EXISTS"  -> exactly one matching attendance already exists and is consistent
      "REVIEW"  -> source-row marker exists but evidence is inconsistent/duplicated
    """
    marker = f"source={SOURCE_FILENAME}/{SOURCE_SHEET}/row{p.source_row};"
    rows = cur.execute(
        """
        SELECT AttendanceId, SessionId, AttendanceMemberKind, MemberDisplayId,
               MemberName, Notes
    ...
    if len(rows) > 1:
        log.add(
            "REVIEW_DUPLICATE_SOURCE_ROW_IN_CRM",
            p,
            detail=(
                f"Found {len(rows)} attendance rows carrying the same migration "
                f"source marker '{marker}'. Manual review required."
            ),
            session_id=session.session_id,
        )
        return "REVIEW"

    attendance_id, existing_session_id, kind, display_id, member_name, notes = rows[0]
    notes = clean_text(notes) or ""

    expected_giveaway = f"giveaway={p.event_number};"
    expected_wellbeing = f"wellbeing_card={p.wellbeing_id or ''};"

    mismatches = []
    if int(existing_session_id) != int(session.session_id):
        mismatches.append(
            f"session changed: existing SessionId={existing_session_id}, "
            f"expected SessionId={session.session_id}"
        )
    if normalize_name(member_name) != normalize_name(p.name):
        mismatches.append(
            f"name changed: existing MemberName='{member_name}', source='{p.name}'"
        )
    if expected_giveaway not in notes:
        mismatches.append(f"missing expected note token '{expected_giveaway}'")
    if expected_wellbeing not in notes:
        mismatches.append(
            f"Wellbeing source evidence changed; expected note token "
            f"'{expected_wellbeing}'"
        )

    if mismatches:
        log.add(
            "REVIEW_EXISTING_SOURCE_ROW_MISMATCH",
            p,
            detail=f"AttendanceId={attendance_id}; " + "; ".join(mismatches),
            session_id=session.session_id,
            member_ref=f"{clean_text(kind) or ''}:{clean_text(display_id) or ''}",
        )
        return "REVIEW"
    ...
    log.add(
        ...
        p,
        detail=(
            f"Source-row idempotency match; AttendanceId={attendance_id}; "
            f"existing migration attendance retained"
        ),
        session_id=session.session_id,
        member_ref=f"{clean_text(kind) or ''}:{clean_text(display_id) or ''}",
    )
...
def try_reviewed_full_override(
    p: Recipient,
    full_by_card,
    log: MigrationLog,
) -> Optional[ResolvedMember]:
    key = (p.event_number, p.source_row, normalize_name(p.name))
    cfg = REVIEWED_FULL_OVERRIDES.get(key)
    if not cfg:
        return None

    actual_wellbeing = normalize_wellbeing_id(p.wellbeing_id) or ""
    expected_wellbeing = normalize_wellbeing_id(cfg.get("expected_wellbeing")) or ""
    actual_email = normalize_email(p.email)
    expected_email = normalize_email(cfg.get("expected_email"))
    actual_gender = (clean_text(p.gender) or "").strip().lower()
    expected_gender = (clean_text(cfg.get("expected_gender")) or "").strip().lower()

    mismatches = []
    if actual_wellbeing != expected_wellbeing:
        mismatches.append(
            f"source Wellbeing changed: expected={expected_wellbeing or '-'} "
            f"actual={actual_wellbeing or '-'}"
        )
    if actual_email != expected_email:
        mismatches.append(
            f"source email changed: expected={expected_email or '-'} "
            f"actual={actual_email or '-'}"
        )
    if actual_gender != expected_gender:
        mismatches.append(
            f"source gender changed: expected={expected_gender or '-'} "
            f"actual={actual_gender or '-'}"
        )

    target_card = normalize_card(cfg["saheli_card"])
    target = full_by_card.get(target_card)

    if target is None:
        mismatches.append(f"reviewed FULL card {target_card} no longer exists in CRM")
    else:
        if normalize_name(target.name) != normalize_name(p.name):
            mismatches.append(
                f"reviewed FULL card {target_card} now belongs to '{target.name}', "
                f"not source name '{p.name}'"
            )
        if expected_gender:
            target_gender = (clean_text(target.gender) or "").strip().lower()
            if target_gender and target_gender != expected_gender:
                mismatches.append(
                    f"reviewed FULL card {target_card} gender changed: {target.gender}"
                )

        expected_target_dob = clean_text(cfg.get("expected_target_dob"))
        if expected_target_dob:
            actual_target_dob = clean_text(target.dob)
            if actual_target_dob != expected_target_dob:
                mismatches.append(
                    f"reviewed FULL card {target_card} DOB changed: "
                    f"expected={expected_target_dob}, actual={actual_target_dob or '-'}"
                )

        expected_target_postcode = clean_text(cfg.get("expected_target_postcode"))
        if expected_target_postcode:
            actual_target_postcode = (clean_text(target.postcode) or "").upper().replace(" ", "")
            expected_postcode_norm = expected_target_postcode.upper().replace(" ", "")
            if actual_target_postcode != expected_postcode_norm:
                mismatches.append(
                    f"reviewed FULL card {target_card} postcode changed: "
                    f"expected={expected_target_postcode}, actual={clean_text(target.postcode) or '-'}"
                )

    if mismatches:
        log.add(
            "REVIEW_REVIEWED_OVERRIDE_VALIDATION_FAILED",
            p,
    ...
    log.add(
        "WARN_REVIEWED_FULL_OVERRIDE",
        p,
        detail=(
            f"FULL card={target.card}; ParticipantID={target.participant_id}; "
            f"{cfg['reason']}"
        ),
        member_ref=f"FULL:{target.card}",
    )

    return ResolvedMember(
        "FULL",
        str(target.participant_id),
        target.card or str(target.participant_id),
        target.name or p.name,
        "REUSE_FULL_BY_REVIEWED_OVERRIDE",
    )
...
def resolve_member(
    ...
    preexisting_name_counts: dict[str, int],
    ...
    preexisting_same_name_count = preexisting_name_counts.get(name_key, 0)

    def create_new_lite(action: str = "CREATED_LITE") -> Optional[ResolvedMember]:
        ...
        log.add(
            ...
            p,
            detail=(
                f"MembershipId={mid}; source Wellbeing ID retained only in "
                "migration/attendance notes, not used as Saheli Card Number. "
                f"Resolution={action}."
            ),
    ...
    # Reviewed mappings are validated again at runtime and fail closed.
    reviewed = try_reviewed_full_override(p, full_by_card, log)
    ...
    # If two rows inside the same giveaway have the same name but different
    # Wellbeing IDs, and there was NO same-name CRM member at database preflight,
    # preserve them as separate source identities. A Lite created for the first
    # row during this preview must not make the second row look pre-existing.
    if (
        len(same_event) > 1
        and len(same_event_distinct_ids) > 1
        and preexisting_same_name_count == 0
    ):
        return create_new_lite("CREATED_LITE_DISTINCT_SOURCE_IDENTITY")
    ...
    # Safe duplicate-profile collapse:
    # exactly one FULL + exactly one LITE with the same exact normalized name
    # and the same non-null DOB is treated as a duplicate CRM identity pair.
    # Reuse the FULL record; do not delete or modify the Lite shadow record.
    if len(full_name_matches) == 1 and len(lite_name_matches) == 1:
        f = full_name_matches[0]
        l = lite_name_matches[0]
        if f.dob and l.dob and f.dob == l.dob:
            return ResolvedMember(
                "FULL",
                str(f.participant_id),
                f.card or str(f.participant_id),
                f.name or p.name,
                "REUSE_FULL_BY_DUPLICATE_PROFILE_EVIDENCE",
            )
    ...
    return create_new_lite("CREATED_LITE")
...
def run_migration(commit: bool, audit_only: bool) -> int:
    ...
    print("Saheli CRM - Bike Giveaway Historical Migration V1.5")
    ...
    try:
        ...
        # Immutable snapshot taken BEFORE this migration inserts any Lite member.
        # This prevents a preview-created Lite from being mistaken for a
        # pre-existing CRM identity later in the same transaction.
        preexisting_name_counts = {}
        for key in set(full_by_name) | set(lite_by_name):
            preexisting_name_counts[key] = (
                len(full_by_name.get(key, [])) + len(lite_by_name.get(key, []))
            )
        ...
        for p in recipients:
            ...
            source_status = source_attendance_status(cur, session, p, log)
            if source_status in {"EXISTS", "REVIEW"}:
                continue
