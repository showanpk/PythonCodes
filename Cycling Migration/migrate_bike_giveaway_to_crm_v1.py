#!/usr/bin/env python3
"""
Saheli CRM - Bike Giveaway Historical Migration V1
==================================================
...
PREVIEW is the default. Use --commit only after REVIEW_* = 0.
"""
...
def resolve_member(
    ...
    # Block a Wellbeing ID that the source itself assigns to different names.
    if p.wellbeing_id and p.wellbeing_id in source_stats["wellbeing_conflicts"]:
        log.add(
            "REVIEW_SOURCE_WELLBEING_ID_CONFLICT",
            p,
            detail=(
    ...
    # Block duplicate same-name rows inside one giveaway when the source IDs differ.
    same_event = source_stats["duplicate_name_same_event"].get((p.event_number, name_key))
    if same_event and len(same_event) > 1:
        ids = sorted({x.wellbeing_id or "-" for x in same_event})
        log.add(
            "REVIEW_DUPLICATE_NAME_WITHIN_GIVEAWAY",
            p,
            detail=f"Same normalized name appears more than once in this giveaway; source IDs={ids}",
        )
        return None

    # If exact same identity appears in two giveaway batches, do not assume two bikes.
    repeat = source_stats["repeat_across_events"].get((name_key, p.wellbeing_id or ""))
    if repeat and len({x.event_number for x in repeat}) > 1:
        log.add(
            "REVIEW_REPEAT_RECIPIENT_ACROSS_GIVEAWAYS",
            p,
            detail="Exact same name/Wellbeing ID appears in multiple giveaway batches.",
        )
        return None
    ...
    if total_name_matches > 1:
        candidates = []
        for x in full_name_matches:
            candidates.append(f"FULL:{x.card or x.participant_id}")
        for x in lite_name_matches:
            candidates.append(f"LITE:{x.membership_id}")
        log.add(
            "REVIEW_AMBIGUOUS_EXISTING_MEMBER",
            p,
            detail="Exact source name matches multiple CRM members: " + " | ".join(candidates),
        )
        return None
...
def run_migration(commit: bool, audit_only: bool) -> int:
    ...
    print("Saheli CRM - Bike Giveaway Historical Migration V1")
    ...
    try:
        ...
        for p in recipients:
            ...
            if member is None:
                # resolve_member already emitted the REVIEW_* reason.
                continue
