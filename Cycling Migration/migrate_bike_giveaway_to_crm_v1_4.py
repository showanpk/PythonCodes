#!/usr/bin/env python3
"""
Saheli CRM - Bike Giveaway Historical Migration V1.4
==================================================
...
V1.4 fixes:
- Keeps all V1.3 safety rules and reviewed member mappings.
- Adds source-row idempotency before member resolution.
- On rerun, an attendance row previously created from the same workbook/sheet/
  source row is recognised directly from its migration note, so duplicate Lite
  identities are not created and same-name source rows do not become ambiguous.
- Existing source-row markers are validated against giveaway, source name,
  Wellbeing ID and target session. Any mismatch becomes REVIEW_* and fails closed.
...
def run_migration(commit: bool, audit_only: bool) -> int:
    ...
    print("Saheli CRM - Bike Giveaway Historical Migration V1.4")
